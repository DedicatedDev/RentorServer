...
class HomeModel extends CI_Model {
	...
	public function home_all_data($user_id)
    {
		$data = array();
        ...
        $cat_result=  $this->db->select('id, name,category_icon,category_color')->from('category')->where('status', 1)->order_by('id','asc')->get()->result();
        $subcat_result = array();
        $cat_subcat_result  = array();

        if(!empty($cat_result)){
        	foreach($cat_result as $cat){
        		$subcat_result =  $this->db->select('id, name,sub_cat_icon')->from('sub_category')->where('status', '1')->where('category_id',$cat->id)->get()->result();
        		$cat_data['id'] = $cat->id;
		...
		foreach ($cat_result as $category)
			{
				...
				$sub_cat_result=  $this->db->select('*')->from('sub_category')->where('status', 1)->where('category_id',$category->id)->order_by('id','asc')->get()->result();
